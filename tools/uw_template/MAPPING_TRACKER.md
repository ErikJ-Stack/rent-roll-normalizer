...
> Generated from `tools/uw_template/registry.json` on 2026-05-23 17:21 UTC.  **Do not edit by hand** — edit `registry.json` and re-run `python tools/uw_template/build_mapping_artifacts.py`.

- Analyzer source: `ALF_Financial_Analyzer_Only.xlsx` (substrate `v0.2.9`)
...
| **Stabilized occupied beds — IL** <br/> `occupied_beds_il` | `UW Output!B71` | `—` | `gap_target` | Template has no occupied-bed split rows. Total occupancy is computed inside Rent Roll Analysis from the pasted rent roll; consider deferring or surfacing as a metadata cell. |
| **Stabilized occupied beds — AL** <br/> `occupied_beds_al` | `UW Output!C71` | `—` | `gap_target` |  |
| **Stabilized occupied beds — MC** <br/> `occupied_beds_mc` | `UW Output!D71` | `—` | `gap_target` |  |
...
| **Total OpEx (excl. mgmt)** <br/> `opex_total_excl_mgmt` | `UW Output!E_or_F63` | `—` | `gap_target` | Template's TOTAL OPERATING EXPENSES (row 114) is inclusive of management fee — no direct row for opex-excl-mgmt. Could derive as N111 + N85. |
...
| **EBITDARM** <br/> `ebitdarm` | `UW Output!E_or_F66` | `T-12 Analysis!N115` | `mapped` |  |
| **EBITDAR** <br/> `ebitdar` | `UW Output!E_or_F67` | `T-12 Analysis!N116` | `mapped` | Template equates EBITDAR with NOI (label A116: 'EBITDAR (= NOI)'). Analyzer treats NOI as a header-only row (UW Output 65) and EBITDAR as a distinct line at row 67. Map E67/F67 directly to N116. |
| **EBITDA** <br/> `ebitda` | `UW Output!E_or_F68` | `—` | `gap_target` | Template has no EBITDA row in Layer 3 (only EBITDARM and EBITDAR/NOI). Either add an EBITDA row to template or drop from writer scope. |
...
- **`Rent Roll Analysis`** (manual) — —: Whole sheet is fed by raw rent-roll paste (row 175+, rows 1-172 are diagnostics over the paste). The Analyzer has RR_Input_Data named range that could programmatically populate the raw block — future enhancement.
...
- EBITDA row — add to template, or drop from writer scope?
- Occupied beds (UW Output row 71) — template has no direct target. Add header cell in Prop Info, or rely on Rent Roll Analysis's internal computation?
- Date header at Rent Roll Analysis!A5 — does it expect RR period date in B5 or D5? Confirm format (yyyy-mm-dd vs Excel date).
- Monthly header cells (T-12 Analysis!B56:M56) — should writer overwrite with actual T-12 months from RR_Period_Date / T12_Period_Date, or leave the hardcoded Apr-25..Mar-26?
