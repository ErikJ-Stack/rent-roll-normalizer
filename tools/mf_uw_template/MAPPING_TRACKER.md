...
> Generated from `tools/mf_uw_template/registry.json` on 2026-06-03 16:21 UTC.  **Do not edit by hand** — edit `registry.json` and re-run `python tools/mf_uw_template/build_mapping_artifacts.py`.
...
- AR aging Q-T: confirm the Bldg-Unit join key is stable across the RR and AR docs (same bldg-unit string format) and how to handle the period mismatch (RR Apr 2026 vs AR Mar 2026).
- Status taxonomy: lock the operator-status -> template-status string map (the COUNTIF wildcards expect 'Occupied No Notice', 'Vacant Unrented Ready/Not Ready', 'Down*', 'Vacant*Leased*'/'Vacant Rented*', 'Model'/'Employee'/'Office').
- Ancillary breakouts W-AK: confirm the Sortable-RR 'Source Data' charge-code grid is the canonical source and define the charge-code -> _StdCOA-bucket dictionary. Is the basic RR ever the only doc available (then W-AK stay blank)?
- T-12 col-P mapping: build the PSI-account -> _StdCOA dictionary. Should it live in a future mf_mappings.py (analogous to ALF's Description_Map) or as a lookup tab in the model?
...
- FIVE MF T-12 FORMATS catalogued (2026-06-03) — the future mf_t12 parser's format detector must handle all: (a) PSI flat (Hidden Lakes: acct#, name, 12 contiguous monthly cols, Total); (b) QuickBooks nested P&L (Blairstone: no acct#, parent/sub indent in col E/F, 12 monthly in ODD cols G–AC, TOTAL in AE, skip 'Total *'/headers); (c) Yardi numbered (Avana: col B = '#####-### - Name', months C–N, total O, skip name-'Total*' roll-ups); (d) Yardi/YSI (Ascend: acct# col A + indented name col B, leaf = indent 5, months C–N, total O); (e) Tzadik/AppFolio name-only (Copeland: no acct#, name col A, months B–M, total N, skip 'Total*'+section headers). KEY: Avana + Ascend share the IDENTICAL Yardi standard chart (41000 Market Rent, 41100 Vacancy, 51010 Mgmt Salaries, 61030 Mgmt Fee, 62xxx Taxes, 63xxx Insurance, 70000-89999 below-the-line) — one account-number dictionary covers every Yardi property.
- COA → _StdCOA SEED BUILT (2026-06-03): tools/mf_uw_template/coa_seed.csv (199 acct_root rules + a 70000-89999 EXCLUDED range + 43 name_regex rules), validated by tools/mf_uw_template/_seed_validate.py at 100% leaf-coverage on Avana/Ascend/Copeland (+ Hidden Lakes via _StdCOA col F and Blairstone hand-map). This is the seed for mf_mappings.py (SPEC-MF §2.4/§2.8). Caveats recorded in tools/mf_uw_template/COA-SEED.md: utility rebills map to Utility Reimbursement but must reconcile by source SECTION (income vs expense contra); below-NOI 7xxxx/8xxxx → EXCLUDED; several deals have no Mgmt Fee / near-zero RE Tax (entity-level → pro-forma); Copeland 'Dues' $539,712.92 is an anomaly flagged for analyst review. Coverage = classification completeness; bucket-correctness integration test (EGI/NOI reconciliation by section) is the parser's next gate.
- Col-U 'Status Flag' (Rent Roll Analysis) semantics still unconfirmed — analyst-input vs derived; deferred to the writer step (SPEC-MF §2.7).
