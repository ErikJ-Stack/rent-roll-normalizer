...
action in the **MF UW Model** (`MF_UW_Model_v25.xlsx`, authored externally in
Excel). Newest at top.
...
| --- | --- | --- | --- | --- | --- |
| 2026-06-29 | [MF UW Model v25 absorption](handoffs/2026-06-29-mf-uwt-v25-absorption.md) | v20 → **v25** (committed to `assets/`) | 0.3.0 → **0.4.0** | **Verified** (MF v0.8.0 / 2026-06-29) — operator-authored, absorbed same session | Operator dropped `MF_UW_Model_v25.xlsx` ("update the template for the mf side"), superseding v20. Verified cell-by-cell: **all four writer target sheets (T-12 Analysis @106, Rent Roll Analysis @273 cols A–AK, Prop Info col A/B, Rental Comps @8) are anchor-identical to v20/v15 → zero concept target moved.** v25 deltas are on non-target sheets + display layers: **`Dashboard`→`Dash` rename, `Data Refresh` removed** (24→23 sheets, several reordered), **RR helper cols trimmed to AL-only** (v20's blank AM–AP dropped; AL outside the writer's A–AK clear band → preserved), Prop Info trailing cols E/F dropped. `xl/metadata.xml` preserved (7→7 `cm`). **New cosmetic drop:** 2 extended (x14) data-validation dropdowns on Rent Roll Analysis that openpyxl drops on save — writer fills those cells with real values regardless; surfaced in the report warning; recover via Excel re-save. Bundled asset + `BUNDLED_MF_MODEL_PATH`/`_VERSION` + writer docstring/warning repointed; `templates.v25` + `targets.v25` verbatim-inherited ×90; `primary_template=v25`; artifacts regenerated; writer test repointed. No operator follow-up required for the mapping. |
