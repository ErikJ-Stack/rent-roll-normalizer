...
**Fix** (`tools/migration/migrate_to_v0216.py`): insert **one row** in `T12 Analytics` at row 92 (immediately after `Auto insurance` at A91), mirroring the A91 row — `A92="Auto Expense"`, `E92=IFERROR(INDEX('T12 Raw Data'!R:R,MATCH("Auto Expense",'T12 Raw Data'!B:B,0)),0)`, `F92==E92`, `G92==F92-E92`. Then run the full-workbook +1 row-shift sweep for every formula referencing `T12 Analytics` rows ≥ 92. Because the new row lands **inside** the original non-labor SUM range, the endpoint auto-extends `E103=SUM(E79:E102)` → (now at `E104`) `=SUM(E79:E103)`, capturing both the new row and the shifted real rows. The EBITDA chain shifts + re-points (`E106=E76+E104`, `E109 EBITDARM=E52-E106`, `E111 EBITDAR=E109-E107`, `E114 EBITDA=E111-E113`), and the 187 cross-sheet single-cell refs into T12 Analytics (Dashboard, UW Output ×34, AR & Collections ×1) re-point automatically. No Monthly Trending / UW Output structural change needed — Monthly Trending already had an `Auto Expense` row (A59), and UW Output's `Total non-labor` (E62) is a *pull* from `'T12 Analytics'!E103` (now `E104`), not a sum of its own lines.

**BL-0001 endpoint-trap cleared by pre-flight:** a verified scan confirmed there are **no cross-sheet *range* refs** into T12 Analytics (all 187 external refs are single cells), **no chart** references T12 Analytics (all 6 Dashboard charts read `Dashboard!` cells), and **no data validation** touches the affected rows. The only within-sheet range crossing row 92 is the non-labor SUM we *want* to extend, so the classic qualified-range-endpoint drift does not apply here.

**Verification:** 21-check verify block all OK; idempotent (gate: `Cover!B8 == "v0.2.16"` AND `T12 Analytics!A92 == "Auto Expense"`); sheet count unchanged at 16; all 16 AZ4 anchors stamped v0.2.16; zip-part inventory identical pre/post (42 parts, no dropped parts — no dynamic arrays in this workbook). **Math proven arithmetically on the populated Homestead fixture** (no LibreOffice; replicated the formula chain in Python from the fixture's cached `T12 Raw Data` values — PRE EBITDARM $1,767,482.75 reproduced the fixture's cached `E108` to the penny, validating the replication): Auto Expense = **$6,061.32**; adding it to the non-labor sum drops EBITDARM **$1,767,482.75 → $1,761,421.43** (−$6,061.32), EBITDAR/EBITDA **$1,417,384.90 → $1,411,323.58** (−$6,061.32) — and **$1,411,323.58 ties to the as-reported NOI** ($1,411,324). Matches the Python engine. All 5 suites in `tests/test_uw_output_model.py` green. Bundled `ALF_Financial_Analyzer_Only.xlsx` updated in place v0.2.15 → v0.2.16; `ANALYZER_SUBSTRATE_VERSION` in `app.py` → `"0.2.16"`.

**Follow-up (not in this release):** the `_AUTO_EXPENSE_DIVERGENCE` whitelist in `tests/test_uw_output_model.py` stays until the gitignored populated Homestead Analyzer fixture is rebuilt against the fixed substrate (needs Excel to evaluate the formulas so cached == engine); only then does cached include Auto Expense and the whitelist can drop. The substrate fix landing here is the prerequisite. (The separate `_REMAP_2P_DIVERGENCE` whitelist retires the same way against a v0.2.15-rebuilt fixture.)
