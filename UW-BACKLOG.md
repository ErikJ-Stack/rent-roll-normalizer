...
- **Shipped in:** the user's local file (informal), then bundled via BL-0021 wholesale-replace (2026-05-19). NOTE: PR #34 (`claude/bl-0020-dashboard-data-link-fixes` branch with `migrate_to_v028.py`) was **closed unmerged** when user chose the wholesale-replace approach. The chart-data-link corrections themselves are present in the bundled Analyzer because the user's local file (which contained the corrections) became the bundled file.
- **Track:** Substrate (Track 3)
...
- **Summary:** Three Dashboard bugs corrected: **(1) Monthly EGI Trend** line chart was plotting **Housekeeping Income** — `Dashboard!C97:C108` referenced `Monthly Trending!B21:M21` (row 21 = Housekeeping Income since v0.1.7), corrected to row 26 (EGI). **(2) Payer Mix — Revenue Share** pie chart was plotting **unit counts** instead of revenue ratios — `Dashboard!F90:F93` referenced `Rent Roll Recon!B40:B43` (COUNTIFS for unit counts), corrected to `I40:I43` (`H/H47` revenue-ratio formulas matching the chart title). **(3) Doughnut chart [1] series range** covered `$O$8:$O$19` (12 rows) but only `O8` + `O15:O19` were populated, rendering 6 empty slices in Excel; corrected by moving data from `O15:O19` up to `O9:O13` (contiguous with `O8`) and shortening the series range to `$O$8:$O$14`. The `migrate_to_v028.py` script and `v028_assets/dashboard_template.xlsx` asset are committed on branch `claude/bl-0020-dashboard-data-link-fixes` for anyone who wants to apply BL-0020 as a targeted patch onto a v0.2.7 baseline workbook in the future. Lesson for future Dashboard work: substrate-migration verification should spot-check chart titles vs source row labels, not just "do refs resolve to populated cells."
