...
- **Shipped in:** substrate v0.2.16 (2026-05-30) via `tools/migration/migrate_to_v0216.py`. Bundled `ALF_Financial_Analyzer_Only.xlsx` updated in place v0.2.15 → v0.2.16; `ANALYZER_SUBSTRATE_VERSION` in `app.py` → `"0.2.16"`.
- **Track:** Substrate (Track 3)
- **Originally surfaced in:** chat 2026-05-29 (UWT v0.8.0 cleanup). The Python engine fix (UWT v0.7.0, 2026-05-28) added `"Auto Expense"` to `dashboard_model._LABELS_NON_LABOR` after confirming the label sums to **$6,061.32 on Homestead = the exact standardized-vs-as-reported NOI gap**. But the **Excel substrate had the same bug**: `T12 Raw Data!B63` carries the `Auto Expense` label (so the GL $ aggregates), yet `T12 Analytics!A79:A102` (the non-labor block feeding `E103=SUM(E79:E102)`) had **no Auto Expense line** — only `Auto insurance` (A91). So the Analyzer's own EBITDARM/EBITDAR/NOI overstated NOI by the Auto Expense amount, and the engine and substrate disagreed on whether Auto Expense was in opex (engine correct, substrate not).
- **Summary (fix):** insert **one row** in `T12 Analytics` at row 92 (immediately after `Auto insurance` A91), mirroring the A91 row — `A92="Auto Expense"`, `E92=IFERROR(INDEX('T12 Raw Data'!R:R,MATCH("Auto Expense",'T12 Raw Data'!B:B,0)),0)`, `F92==E92`, `G92==F92-E92`. Full-workbook +1 row-shift sweep for every formula referencing T12 Analytics rows ≥ 92. The new row lands **inside** the non-labor SUM range, so the endpoint auto-extends `E103=SUM(E79:E102)` → (now `E104`) `=SUM(E79:E103)` — capturing both the new row and the shifted real rows. EBITDA chain shifts + re-points (`E106=E76+E104`, `E109 EBITDARM=E52-E106`, `E111 EBITDAR=E109-E107`, `E114 EBITDA=E111-E113`); the 187 cross-sheet single-cell refs into T12 Analytics (Dashboard, UW Output ×34, AR & Collections ×1) re-point automatically. No Monthly Trending / UW Output structural change needed — Monthly Trending already had an `Auto Expense` row (A59), and UW Output's `Total non-labor` (E62) is a *pull* from `'T12 Analytics'!E103` (now `E104`), not a sum of its own lines.
- **BL-0001 endpoint-trap cleared by pre-flight:** verified scan confirmed **no cross-sheet *range* refs** into T12 Analytics (all 187 external refs are single cells), **no chart** references T12 Analytics (6 Dashboard charts read `Dashboard!` cells only), **no data validation** touches the affected rows. The only within-sheet range crossing row 92 is the non-labor SUM we *want* to extend, so the qualified-range-endpoint drift does not apply.
- **Verification:** 21-check verify all OK; idempotent (gate: `Cover!B8 == "v0.2.16"` AND `T12 Analytics!A92 == "Auto Expense"`); sheet count unchanged at 16; 16 AZ4 anchors stamped v0.2.16; zip-part inventory identical pre/post (39 parts). **Math proven on the populated Homestead fixture** (no LibreOffice): Auto Expense = $6,061.32; adding it drops EBITDARM $1,804,812.66 → $1,798,751.34 (−$6,061.32), EBITDAR/EBITDA likewise −$6,061.32 — matching the engine to the penny.
- **Follow-up (not in this release):** the `_AUTO_EXPENSE_DIVERGENCE` whitelist in `tests/test_uw_output_model.py` stays until the gitignored populated Homestead Analyzer fixture is rebuilt against the fixed substrate (needs Excel to evaluate formulas so cached == engine); only then can the whitelist drop. The substrate fix landing here is the prerequisite. (The separate `_REMAP_2P_DIVERGENCE` whitelist retires the same way against a v0.2.15-rebuilt fixture.)

### [BL-0027] README.md modernization — current versions + Tracks 4/5 + AR
- **Shipped in:** 2026-05-27 (README refresh) + 2026-05-29 versions-table catch-up (UWT v0.8.0 cleanup pass) + 2026-06-03 third pass (versions → substrate v0.2.16 / UWT v0.9.0; **MF product line added** — versions row, intro mode-selector note, track map, dedicated "MF (multifamily) mode" subsection, `mf_*` files + UW Model assets in project layout, SPEC-MF / CHANGELOG-MF in further-reading; migration list collapsed to the v0.1.5 → v0.2.16 chain).
...
- **Track:** Cross-cutting — Track 1 (parser/writer) + Track 3 (substrate). Cross-track work authorized by user 2026-05-25 at chat start.
- **Originally surfaced in:** User report 2026-05-25 — "in the homestead village RR, there are move outs, this isn't captured in the normalizer and not reflected in the analyzer. I need this captured for exposure." Diagnosis: (1) Homestead's three "Vacant w/ Prelease" units (A4, F1, F2) were silently collapsing to plain `Vacant` because the `\bvacant\b` rule fired before any prelease rule, overstating exposure on Rent Roll Recon Section A. (2) `Move-out Date` was captured into `Rent Roll Input!W` since v1.16.0 / substrate v0.1.10 but no Analyzer formula read it — no underwriting "exposure" view existed.
- **Summary:** Three coordinated pieces shipped:
    1. **Parser (RR v1.18.0)** — `mappings.py` adds `(r"\bprelease", "Preleased")` immediately before `\bvacant\b` so "Vacant w/ Prelease" hits Preleased first (3 units on Homestead). `normalizer.py` adds `prelease_date` to `FIELD_PATTERNS` matching Homestead's bare `Preleased` column header (semantically the date the prelease was signed). `CONDENSED_COLUMNS` gets a 31st column (`Preleased Date`, AE on Condensed_RR). `analyzer_rr_writer.py` routes Preleased Date to `Rent Roll Input!AI` (AH=34 is reserved for the v0.1.13 Total Ancillary $ formula). `analyzer_rr_translator.py` docstring update only — `Preleased` passes through unchanged.
    2. **Substrate (v0.2.13)** — `Rent Roll Input!E7:E606` DV list extended `"Occupied,Vacant,Notice,Eviction"` → `"Occupied,Vacant,Notice,Eviction,Preleased"`. `Rent Roll Input!AI4` = "Preleased\nDate" header (Q4-styled). `Rent Roll Recon` Section N appended at rows 178-198 (pure append, no `insert_rows` — avoids the BL-0001 qualified-range-endpoint shift trap). **N1 point-in-time** (rows 180-189): Occupied / Notice / Vacant / Preleased / Total beds / Gross exposure (Notice + Vacant) / Net exposure (Gross − Preleased) / Net exposure %. **N2 forward NTV departures** (rows 191-198): ≤30d / 31-60d / 61-90d / 91+d / No date or past / Total Notice sanity check. Time windows computed against the period selected at `Rent Roll Recon!B2`.
    3. **Net = Notice + Vacant − Preleased** — conventional ALF UW exposure framing. Preleased units offset gross because the lease is signed and the unit is on its way to being filled, not a risk-bearing vacancy. Gross and Net surfaced side-by-side (rows 187 + 188) so a reader sees the offset rather than just the net.
- **End-to-end on Homestead fixture** (period 2026-04-24, 176 beds): Status counts 128/40/5/3 (was 128/43/5/0 — 3 units moved from Vacant to Preleased). N1 Net exposure: 7 IL / 18 AL / 17 MC = 42 total (23.9%); gross 8/20/17 = 45. N2 ≤30d: 3 NTVs (Julius Mims 4/30, Peggy Salger 4/30, Thomas Winterbury 5/13 — all AL); No date / past: 2 NTVs (Hedenburg, Stowe — neither has a scheduled date). Sanity holds: N2 Total Notice = N1 On notice = 5.
- **Verification:** Migration script 8-check verify block all OK (Cover B8 / DV / AI4 / Section N spot-checks / Preleased formula references `E="Preleased"` / Net formula references Preleased row / sheet count = 16 / 16 AZ4 anchors stamped v0.2.13). Idempotency confirmed (re-run on v0.2.13 output → "No-op"). Parser smoke on Homestead: 176 rows, 4-status split. Python equivalent reproduction of Section N formulas matches expected Excel evaluation.
...
### [BL-0024] Dashboard blended-vs-segment formula mis-references
- **Shipped in:** substrate v0.2.12 (2026-05-25) via `tools/migration/migrate_to_v0212.py`. Bundled `ALF_Financial_Analyzer_Only.xlsx` updated in place v0.2.11 → v0.2.12.
- **Track:** Substrate (Track 3)
- **Originally surfaced in:** chat 2026-05-24 during Track 5 (webapp Dashboard) build. `dashboard_model.py` correctly computes blended community occupancy, blended ADR, and blended RevPOR but the xlsx `Dashboard` sheet's three headline-tile cells (B6/F20/K6) — and 9 derivative cells with the same bug pattern — were diverging. Diagnosis: tiles labeled "Normalized community occupancy" / "Blended ADR / day" / "Normalized RevPOR per resident" reference `'T12 Analytics'!F134` / `F140` / `F143`, but F134 = AL-only occupancy (`=C11/C6`), F140 = MC-only ADR (`=D20/(D11*12)`), F143 = MC-only RevPOR (`=(D20+D27)/(D11*12)`). T12 Analytics col A even labels those F-cells "— AL" / "— MC" — the bug was on the Dashboard pulls, not on the T12 Analytics source.
- **Summary:** Twelve Dashboard cells rewritten to compute blended directly from T12 Analytics column-E primitives (E6 = total licensed beds, E11 = total stabilized occupied beds, E20 = blended stabilized annual base rent, E27 = blended stabilized LOC revenue). Affected cells: **B6** (OCCUPANCY tile), **B8** (OCCUPANCY status text), **C21** (Community occupancy % row), **D35** (Community occupancy card), **E55** (Occupancy gap-to-market delta), **G55** (Occupancy risk flag emoji), **H55** (Occupancy risk flag text), **P5** (Upper-right Blended anchor), **F20** (Blended ADR tile), **K6** (REVPOR tile), **K8** (REVPOR status text), **F21** (RevPOR detail row). All rewrites wrap inline division in `IFERROR(...,"—")` so div-by-zero on an empty workbook renders the same em-dash placeholder the old formulas produced; threshold-comparison shapes (`B8`, `G55`, `H55`) preserve the existing ✓/⚠/✗ branches and "— Source not populated" fallback exactly. **Homestead populated fixture before/after:** OCCUPANCY 64.5% → 72.7%; ADR $6,802 → $4,546; REVPOR $6,802 → $4,562. **Inline approach** (12 cells × E-primitive math) chosen over **add-rows-to-T12-Analytics** (would have added "Blended ADR" + "Blended RevPOR" cells to Section 5 KPI Dashboard and pointed Dashboard at them) — inline keeps the change blast radius to Dashboard only, no T12 Analytics surface change. Migration via `migrate_to_v0212.py` (12 cell rewrites + 17 version stamps, 5-check + 12-per-cell verify, idempotent — gate checks `Cover!B8 == "v0.2.12"` AND `Dashboard!B6` formula does NOT contain substring `"F134"`). Each cell patch is also independently self-idempotent via "current formula contains buggy F134/F140/F143 substring" check; already-fixed cells are skipped. 6 charts preserved, 75 merged ranges preserved, hidden sheet states preserved (AR & Collections, Workbook Health), AR variance tile at K10:L13 unaffected. **Cross-pipeline impact:** the bundled xlsx Dashboard tile numbers now match what `dashboard_model.py` computes in the Streamlit webapp Dashboard tab — resolving the cross-pipeline discrepancy that surfaced the bug.
