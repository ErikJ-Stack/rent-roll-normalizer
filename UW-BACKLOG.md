...
- **Track:** Substrate (Track 3) · no target version assigned
- **Surfaced in:** User report 2026-05-16 ("rent roll input tab has a missing label on row 4") against the populated Homestead v0.2.3 Analyzer.
- **Description:** When the AH "Total Ancillary $" column was added in substrate v0.2.2, the header cell received correct white-bold font but `fill_type=None` (transparent). White-on-default renders as a blank cell — the column header is invisible. By the substrate's existing header palette, AH is a computed column (formula `=IFERROR(V+AC+AD+AE+AF+AG,0)`) so the green `FF1F6B52` fill matching T4/U4 is correct, not the navy `FF1F3864` used for input columns.
- **Status — handled manually by user, NOT via substrate migration.** Originally implemented as substrate v0.2.4 Step A in [PR #26](https://github.com/ErikJ-Stack/rent-roll-normalizer/pull/26) bundled with BL-0017; PR was closed unmerged on 2026-05-16 after the user decided to handle the formatting manually in Excel. The fix is trivially recreatable from this description if a future chat needs it (one-cell `PatternFill(start_color="FF1F6B52", end_color="FF1F6B52", fill_type="solid")` on `Rent Roll Input!AH4`), but **do not re-attempt as a substrate migration without re-confirming the user wants it that way** — they explicitly chose the manual path. The abandoned v0.2.4 migration script lives at `claude/serene-panini-3ad41d` on origin (commit `fac129d`) if anyone wants to reference the implementation.

### [BL-0017] Workbook-wide "intentionally blank" visual convention
- **Track:** Substrate (Track 3) · no target version assigned
- **Surfaced in:** User report 2026-05-16 ("T12 Analytics E36:E37 doesn't add up") against the populated Homestead v0.2.3 Analyzer. Diagnosis identified T12 Analytics E36/G36 as storing the literal 3-character string `"-"` (with quote chars in the text payload), rendering as `"-"` with visible quote marks in Excel. Pre-flight migration sweep revealed the same literal in 144 cells workbook-wide: 2 on T12 Analytics (E36, G36), 1 on Rent Roll Recon (D109 — MC "Other / unmapped" Avg Care Level), 141 on UW Output (cols B/C/D × rows {8-12, 22-28, 30-36, 38-56, 58-60, 62-64, 66-68} — per-care-type columns adjacent to total-only formulas). All 144 share the same "intentionally blank, not just missing data" design intent.
- **Description:** No coherent visual convention currently exists in the substrate for "blank by design" cells. Proposed treatment (chosen by user via 3-option preview on 2026-05-16): `value="—"` (em-dash plain text) + `fill=PatternFill(start_color="FFF2F2F2", end_color="FFF2F2F2", fill_type="solid")` (light gray) + `font color=FFA0A0A0` (medium gray, preserving size/bold/italic) + `horizontal alignment=center` (preserving vertical/wrap/indent). New user-facing rule: gray + em-dash = "blank by design"; truly empty = "data not yet populated".
