...
The **MF UW Model** (`MF_UW_Model_v20.xlsx`) is the downstream per-deal
...
The committed reference copy lives at `assets/MF_UW_Model_v20.xlsx` (v15 retained
for the override path / history). The operator's canonical copy lives at
`Deals/Acquisition/_Template/MF Templates/MF_UW_Model_v20.xlsx`.

**v20 (binding from 2026-06-21)** is layout-compatible with v15 on all four
write-target sheets (T-12 Analysis Layer 1 @106, Rent Roll Analysis grid @273
cols A–AK, Prop Info col B, Rental Comps @8) — the writer's anchors are
unchanged. v20 adds a `Dashboard` sheet (idx 1; sheet_count 23 → 24), per-row
chart-helper formula columns AL–AP on Rent Roll Analysis (formula-derived,
outside the writer's clear band), and — unlike v15 — an `xl/metadata.xml` part
for Excel-365 dynamic arrays (preserved on save by the writer's
`_restore_dynamic_arrays` repair, now active).

### 1.2 Model shape (v15 geometry, unchanged in v20)

23 sheets (24 in v20 with `Dashboard`). The full model spans Cover, Inputs, Prop Info, Data Refresh, Data
...
concepts to version-keyed model targets (`templates.v15` + `templates.v20`; v20
is primary). 90 concepts across the intake + OM paths. Status rollup at v0.3.0:
63 `mapped` / 5 `proposed` / 21 `gap_source` / 1 `derived`. v20 absorption added
`targets.v20` as a verbatim inherit of `targets.v15` on all 90 concepts (no
target moved — see `handoffs/2026-06-21-mf-uwt-v20-absorption.md`).
...
- **MF** product line — `vX.Y.Z`, current **v0.7.0** (MF UW Model v20
  absorption — bundled asset + writer + app repointed; `Dashboard` sheet +
  AL–AP helper cols + `metadata.xml` dynamic arrays preserved on round-trip;
  v0.6.0 MF Dashboard webapp tab; v0.5.2 T-12 Yardi numeric date headers).
- Registry `registry_version` tracks the registry data separately; currently
  `0.3.0`, mapped against model `templates.v20` (primary; `templates.v15`
  retained).
